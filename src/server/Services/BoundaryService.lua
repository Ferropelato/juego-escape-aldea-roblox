...
function BoundaryService.init()
	...
	RunService.Heartbeat:Connect(function()
		...
		for _, player in Players:GetPlayers() do
			...
			local islandCenter = ISLAND_CENTERS[data.currentIsland] or ISLAND_CENTERS.Island1_Tropical

			local flat = Vector3.new(hrp.Position.X - islandCenter.X, 0, hrp.Position.Z - islandCenter.Z)
			...
			if dist > MAX_RADIUS then
				...
				if cf then
					...
					hrp.CFrame = CFrame.new(islandCenter + Vector3.new(0, 15, 0))
				end
